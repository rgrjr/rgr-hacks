...
(defun rgr-show-squid-log-date ()
  ...
  (let ((date-string nil))
    (save-excursion
      ...
      ;; (setq milliseconds (string-to-number (match-string 2)))
      (setq date-string (match-string 1)))
    (let* ((low-start (- (length date-string) 6))
	   (millions (string-to-number (substring date-string 0 low-start)))
	   (units (string-to-number (substring date-string low-start)))
